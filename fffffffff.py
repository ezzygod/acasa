...
def p_expresii(c):
    '''expresie : termen
                ...
                | NEGATE termen'''
    if len(c) == 2:
        c[0] = c[1]  # for a single term
    elif c[1] == 'neg':
        c[0] = -c[2]
        istoric_text.append(f"neg {c[2]} = {c[0]}")
    else:
...
Button(calculator, text="Cist", width=5, height=2, command=clear_istoric).grid(row=3, column=4)
#buton negare numar
Button(calculator, text="neg", width=5, height=2, command=lambda b=negare: buton_click(b)).grid(row=4, column=4)
